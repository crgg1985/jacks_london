...
<option></option>
	...
	$result = $db->prepare("SELECT * FROM category");
		$result->bindParam(':category_id', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
	?>
		<option value=<?php echo $row['category_id']; ?>><?php echo $row['descripcion']; ?></option>
	<?php
	}
	?>
</select><br>

<span>Nombre Marca : </span><input type="text" style="width:265px; height:30px;" name="code" ><br>
